<div class="relative h-64 overflow-hidden">
    ...
    <div class="absolute top-4 right-4 flex flex-col gap-2 items-end">
        <span class="bg-[#2ecc71] text-white text-[10px] font-bold px-3 py-1 rounded-sm uppercase">Guaranteed
            Departure</span>
    </div>
...
<template x-teleport="body">
    <div x-show="openForm" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
        ...
        class="fixed inset-0 z-[999] flex items-center justify-center bg-black/60 px-4" style="display: none;">
            ...
            <form @submit.prevent="submitBooking" class="w-full flex flex-col md:flex-row h-full">
                <div class="md:w-3/5 p-6 md:p-10 border-r border-gray-100 h-full overflow-y-auto bg-white">
                    ...
                    <div class="flex gap-3 mb-8 text-sm font-medium">
                        <span @click="step = 1" class="cursor-pointer hover:text-[#074b83]"
                            :class="step === 1 ? 'text-[#074b83] font-bold underline' : 'text-gray-400'">Package</span>
                        <span>&rarr;</span>

                        <span @click="selectedStart ? step = 2 : null" :class="{
              'text-[#074b83] font-bold underline': step === 2,
              'text-gray-400 cursor-not-allowed opacity-50': !selectedStart,
              'text-gray-400 cursor-pointer hover:text-[#074b83]': selectedStart && step !== 2
          }">Info</span>
                        <span>&rarr;</span>

                        <span @click="isStep2Valid ? step = 3 : null" :class="{
              'text-[#074b83] font-bold underline': step === 3,
              'text-gray-400 cursor-not-allowed opacity-50': !isStep2Valid && step < 3,
              'text-gray-400 cursor-pointer hover:text-[#074b83]': (isStep2Valid || step > 3) && step !== 3
          }">Insurance</span>
                        <span>&rarr;</span>

                        <span @click="isStep2Valid ? step = 4 : null" :class="{
              'text-[#074b83] font-bold underline': step === 4,
              'text-gray-400 cursor-not-allowed opacity-50': !isStep2Valid && step < 4,
              'text-gray-400 cursor-pointer hover:text-[#074b83]': isStep2Valid && step !== 4
          }">Payment</span>
                    </div>
                    <div class="pb-10">
