<div class="md:w-2/5 bg-gray-50 p-6 md:p-10 flex flex-col h-full overflow-y-auto">
    <div class="flex justify-between items-start mb-6">
        ...
        <div class="relative">
            <button type="button" @click="showCurrency = !showCurrency"
                ...
                <img :src="`https://flagcdn.com/w20/${currencyRates[selectedCurrency].flag}.png`"
                    class="w-5 h-auto rounded-sm">
                <span x-text="selectedCurrency"></span>
            </button>
            <div x-show="showCurrency" @click.away="showCurrency = false"
    ...
    <div class="space-y-4 mb-8">
        <div class="text-sm text-gray-600 font-medium pb-2 border-b border-gray-200" x-show="selectedStart">
            <span x-text="selectedStart.toLocaleDateString('en-US', {month:'short', day:'numeric'})"></span>
            -
            <span x-text="selectedEnd.toLocaleDateString('en-US', {month:'short', day:'numeric', year:'numeric'})"></span>
        </div>
        ...
        <div class="flex justify-between text-gray-700" x-show="includeInsurance">
        ...
        <div class="flex justify-between text-green-700 font-bold bg-green-50 p-3 rounded-lg border border-green-100 items-center">
            <span>Due at Booking</span>
            <div class="text-right">
                <span x-text="formatPrice(deposit)"></span>
    ...
    <div class="space-y-4 pt-4 border-t border-gray-200">
        ...
        <label class="flex items-start gap-3 cursor-pointer p-3 rounded-lg border border-gray-200 bg-white hover:border-[#074b83] transition"
               :class="paymentOption === 'deposit' ? 'ring-2 ring-[#074b83] border-transparent' : ''">
            <input type="radio" x-model="paymentOption" value="deposit" class="mt-1 w-5 h-5 text-[#074b83]">
            <div>
        ...
        <label class="flex items-start gap-3 cursor-pointer p-3 rounded-lg border border-gray-200 bg-white hover:border-[#074b83] transition"
               :class="paymentOption === 'full' ? 'ring-2 ring-[#074b83] border-transparent' : ''">
            <input type="radio" x-model="paymentOption" value="full" class="mt-1 w-5 h-5 text-[#074b83]">
            <div>
