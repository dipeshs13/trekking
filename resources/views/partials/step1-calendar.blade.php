<div x-show="step === 1">
                                    ...
                                    <div class="grid grid-cols-7 gap-1 text-center text-sm mb-6">
                                        <template x-for="head in ['Su','Mo','Tu','We','Th','Fr','Sa']"><div class="text-gray-500 font-medium py-1" x-text="head"></div></template>
                                        <template x-for="(day, index) in currentMonthDays" :key="currentMonthName + index">
                                            <button type="button" @click="selectDate(day)" :disabled="!day.available || !day.date"
                                                :class="{
                                                ...
                                                }" class="aspect-square rounded-full flex items-center justify-center text-sm transition" x-text="day.date || ''">
                                            </button>
                                        </template>
                                    </div>
                                    <div class="mb-8">
                                        <h3 class="text-xl font-semibold mb-3">Number of People</h3>
                                        <select x-model="participants" class="border rounded-lg px-4 py-2.5 w-24 text-lg outline-none focus:ring-2 focus:ring-[#074b83]/20">
                                            <template x-for="i in 24"><option :value="i" x-text="i"></option></template>
                                        </select>
                                    </div>

                                    <button type="button" @click="goToParticipantInfo()" class="mt-4 w-full md:w-auto self-end bg-[#2ecc71] hover:bg-[#27ae60] text-white font-bold py-4 px-10 rounded-lg text-lg transition shadow-lg">Continue</button>
                                </div>